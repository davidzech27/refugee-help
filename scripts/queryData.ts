	...
	const queryLoop = new QueryLoop({
		rephraseQueryPrompt: ({ query, previousQuery, previousCompletion }) => {
			if (!previousQuery || !previousCompletion) {
				return `Rephrase the following question express the user's intent in a clearer manner. Add additional content if necessary to improve the completeness of the question. Keep in mind that the user is most likely currently attempting to immigrate to the US. Do not prefix the rephrased query.

Question: ${query}`;
			} else {
				return `Based the user's previous question and answer, rephrase the user's new question express the user's intent in a clearer manner. Add additional content if necessary to improve the quality and completeness of the question. Do not prefix the rephrased query.
